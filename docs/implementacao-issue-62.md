...
`ClinicalMessages` integra busca da equipe, badge, seleção de conversa, resposta, histórico e correção no detalhe profissional. O componente reutiliza `MentionTextarea`; sua única ampliação foi um rótulo acessível. Não há dados simulados, edição em lugar ou exclusão.
...
Em 14 de agosto de 2026, a validação executou Ruff, 75 testes backend, consistência Alembic, TypeScript, 48 testes frontend em treze arquivos, formatação e build de produção. `api` e `web` foram reconstruídos e ficaram saudáveis com a migração aplicada.
