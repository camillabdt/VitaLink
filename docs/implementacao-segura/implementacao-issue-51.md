...
Esta fatia vertical conecta a recuperação de senha, a recuperação reforçada do segundo fator, a troca autenticada de senha, a listagem e o encerramento de sessões ao frontend local, à API e ao PostgreSQL.
...
Os testes backend usam PostgreSQL e Mailpit reais. Dublê foi usado apenas para o relógio, fronteira explicitamente permitida por `docs/implementacao-segura/etapa5-verificacao-de-seguranca.md`. Os testes de componentes substituem somente a fronteira HTTP pública.
