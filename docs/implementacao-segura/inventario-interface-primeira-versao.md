...
Este inventário transforma a exigência de “todas as telas funcionais” em critérios verificáveis. O código em `VitaLink Health Management App/` é a fonte da interface atual e está integrado à API nos fluxos documentados. Partes do painel do paciente ainda renderizam dados de `mockData.ts`; esses trechos são demonstrações estáticas e não comprovam persistência ou integração com a API.

Segurança, domínio e acessibilidade orientam a evolução da interface. Toda mudança relevante deve ser mencionada na issue e demonstrada na revisão visual.
...
| Destino                     | Implementação atual                                      | Decisão para a primeira versão                              | Comportamento verificável                                                                    |
| --------------------------- | -------------------------------------------------------- | ----------------------------------------------------------- | -------------------------------------------------------------------------------------------- |
...
| Componente                                      | Controles e estados observados                                                                                                                                                          | Decisão                                                                                                                                                      | Destino                            |
| ----------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------- |
...
As issues com frontend devem citar os arquivos atuais que reutilizam e incluir comparação nos viewports de 390 × 844, 768 × 1024 e 1440 × 900. Devem preservar os tokens de cor, tipografia, espaçamento, bordas, hierarquia, sidebar, cards e padrões de formulário da aplicação, ressalvados ajustes documentados.
