...
Este documento registra o contrato usado para implementar a primeira versão executável do VitaLink. Ele consolida as decisões aprovadas durante o Grill realizado em 13 de agosto de 2026 e transforma a documentação de segurança em comportamentos, componentes e critérios verificáveis.

O plano é histórico e normativo. O estado consolidado de código, testes e pendências está no [índice de implementação segura](README.md) e na [Etapa 5](etapa5-verificacao-de-seguranca.md).
...
4. O [inventário da interface](inventario-interface-primeira-versao.md) e o código em `VitaLink Health Management App/`, que registram a interface implementada.

As regras de segurança e de domínio prevalecem sobre decisões apenas visuais. O frontend deve preservar a identidade visual, a organização e os padrões de interação da aplicação, com os ajustes necessários para segurança, acessibilidade, responsividade e coerência funcional.
...
### Regras obrigatórias da interface
...
| ---- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------- |
| DI01 | Manter o frontend React, TypeScript, Vite e Tailwind como implementação da interface.                                                                         | Grill; código local             |
...
| DI22 | Reutilizar estrutura, componentes, tokens e padrões da aplicação em toda issue com frontend; desvios exigem segurança, acessibilidade ou decisão documentada. | Grill; código local             |
...
- layout responsivo conforme o padrão visual da aplicação;
...
- identidade visual permanece coerente entre as telas implementadas;
...
1. Paciente cadastra, confirma e-mail, ativa TOTP e entra pelo frontend implementado.
...
10. A suíte acumulada comprova segurança, acessibilidade, carga, backup, restauração e coerência visual.
...
- todas as telas mantidas na aplicação usarem a API e dados persistidos;
